...
const Regist = ({ history }) => {
    ...
    const [isReadOnly, setIsReadOnly] = useState(false); // 소셜로그인으로 넘어왔을때 이름을 바꿀 수 없게 함

    useEffect(() => {
        const storedUserName = window.localStorage.getItem('userName');
        if (storedUserName) {
            setUserName(storedUserName);
            setIsReadOnly(true); // 소셜 로그인으로 넘어온 경우 읽기 전용으로 설정
        }
    }, []);
                        ...
                        <div className='register-body'>
                            ...
                            <div className='input-register'>
                                ...
                                <input type="text" text="이름" id="name" value={userName}
                                    readOnly={isReadOnly} onChange={handlerChangeUserName}
